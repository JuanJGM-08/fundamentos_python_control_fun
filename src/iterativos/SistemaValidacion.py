...
# En este ejemplo, se define una función 'validar_formulario()' que toma un diccionario de datos como argumento. La función primero verifica que los campos requeridos ("nombre", "email" y "edad") estén presentes y no estén vacíos. Si falta algún campo o está vacío, se agrega un mensaje de error a la lista 'errores' y se utiliza 'break' para salir del bucle inmediatamente. Si todos los campos requeridos son válidos, se ejecuta el bloque 'else' asociado al bucle, donde se realizan validaciones adicionales para el formato del email y la edad. Además, se verifica si el campo opcional "telefono" está presente y, si es así, se valida que contenga solo dígitos. Finalmente, la función devuelve un diccionario indicando si el formulario es válido o no, junto con cualquier error encontrado. Al probar la función con diferentes formularios, se puede observar cómo maneja tanto casos válidos como casos con errores en los datos ingresados.